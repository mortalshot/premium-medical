...
const defaultRoot = resolve(process.cwd(), 'src')
		...
		transformIndexHtml: {
			...
			handler: async (html, { filename, server }) => {
				...
				if (pluginOptions.extend) {
					plugins.push(posthtmlExtend({ root: pluginOptions.root ? pluginOptions.root : defaultRoot, ...pluginOptions.extend }))
				}
				if (pluginOptions.include) {
					plugins.push(posthtmlInclude({ root: pluginOptions.root ? pluginOptions.root : defaultRoot, ...pluginOptions.include }))
				}
				if (pluginOptions.fetch) {
					plugins.push(posthtmlFetch({ root: pluginOptions.root ? pluginOptions.root : defaultRoot, ...pluginOptions.fetch }))
				}
				if (pluginOptions.expressions) {
					plugins.push(posthtmExpressions({ root: pluginOptions.root ? pluginOptions.root : defaultRoot, ...pluginOptions.expressions }))
				}
