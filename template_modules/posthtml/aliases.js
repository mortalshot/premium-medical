 ...
 * @param {boolean} [options.prependDot=false] - добавлять точку ('.') в начале пути.
 * @param {boolean} [options.normalizePath=true] - Нормализовать путь (убирать двойные слэши).
 * @param {boolean} [options.sortAliases=true] - Сортировать ли Алисы по длине (длиннее первыми).
 * @param {boolean} [options.preserveOriginal=true] - Возвращать ли оригинальные данные, если алиасов нет.
 * @param {Function} [options.transformReplacement] - Функция для преобразования значения замены.
...
const replaceAliases = (data, { prependDot = false, normalizePath = true, sortAliases = true, preserveOriginal = true, transformReplacement } = {}) => {
	...
	// Если алиасов нет и preserve Original включен, возвращаем оригинальные данные
	...
	if (typeof data === 'string') {
		let result = data
		// Сортируем алиасы по длине (длиннее первыми), если sortAliases включен
